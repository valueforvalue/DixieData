...
- **debug: archive trace.Log / ring-buffer benchmark suite (issue #550 evidence)**. New `internal/debug/debug_bench_test.go` captures the `BenchmarkRingBuffer_PushEmpty` + `BenchmarkRingBuffer_PushAtCapacity` benchmarks cited in issue #550's release-build cost analysis. Test-only file (no production code change); run with `go test -short -bench BenchmarkRingBuffer -benchtime=1s -run ^$ ./internal/debug/...`. Kept in tree so the numbers in #550 (1.5 KiB static-link delta, ~10 ns per call at Info floor, ~9.2 μs per call at Debug floor, 3.6 μs/s release overhead at 18 call sites × 20 req/s) are re-verifiable on the next Go release. CI does not yet run the bench suite; the `internal/debug/...` test gate in `.github/workflows/test.yml` runs only the unit tests. Future maintainers questioning the #550 numbers can `cd internal/debug && go test -bench .` to reproduce.
