...
- **release: CHANGELOG [Unreleased] sweep applied (#624, partial)**. Ran `node scripts/release-changelog-sweep.mjs --apply` per the issue body. Inserted 94 draft bullets (covering commits between `v1.2.54` and HEAD) under a single `### Maintenance` header at the end of the `[Unreleased]` block. **Bug fix surfaced**: the slice-1 script used `changelog.lastIndexOf('### Maintenance')` which landed on the most recent historical release's Maintenance section (e.g. `v1.2.28`) instead of the [Unreleased] block. Fixed by anchoring the lookup to the `## [Unreleased]` header. The fixed script writes into the FIRST `### Maintenance` section within the [Unreleased] block. **Operator follow-up (the durable work)**: re-categorize the 94 bullets into the correct `### Added` / `### Changed` / `### Fixed` / `### Maintenance` bucket per the commit-subject prefix (`feat` → Added, `fix` → Fixed, everything else → Maintenance). Roughly 50 are `feat:` (Added), 27 are `fix:` (Fixed), 17 are `chore`/`docs`/`bench`/`perf`/`refactor`/`test` (Maintenance) per a quick eyeball of the commit log. ~15-30 minutes of operator work. The 94 bullets are clearly marked `(auto-sweep)` so a future re-categorize pass is easy to script. **Files**: 1 (`scripts/release-changelog-sweep.mjs` bug fix on the section lookup; `CHANGELOG.md` 94-bullet auto-sweep dump under [Unreleased]; this bullet).
...
- **release: auto-sweep CHANGELOG [Unreleased] for 94 missing issue(s) (2026-07-18, scripts/release-changelog-sweep.mjs --apply)**.
  - **A11y: associate form labels with inputs via for/id pairs (#51)** (auto-sweep). feat(a11y): associate form labels with inputs via for/id pairs (#51)
  - **Density: reduce vertical scroll on data rows + buttons (#56)** (auto-sweep). feat(ui): tighten browse filters, calendar cells, share cards, and radio density (#56)
  - **Bulk export: emit single sorted PDF instead of folder of per-record PDFs (#64)** (auto-sweep). fix(export): bulk export emits single sorted PDF instead of folder of PDFs (issue #64)
  - **Bulk export: wire group divider pages for all four GroupBy* options (#65)** (auto-sweep). feat(export): emit group divider pages in bulk PDF export (issue #65)
  - **Bulk export: capture baseline performance metrics for typst path (#66)** (auto-sweep). bench(export): capture baseline metrics for bulk typst PDF export (issue #66)
  - **Bulk export: eliminate per-record typst cold-start overhead (#67)** (auto-sweep). perf(export): parallelize bulk image staging + per-phase instrumentation (issue #67)
  - **Bulk export: split PrintSettings.Template into single vs bulk template fields (#68)** (auto-sweep). feat(export): split PrintSettings.Template into SingleRecordTemplate + BulkTemplate (issue #68)
  - **dixiedata-tune: make the tool actually mirror DixieData's PDF export (currently broken) (#69)** (auto-sweep). chore(tune): gitignore built binary at tools/tune/bin/ (issue #69 step 7)
  - **docs(cleanup): delete superseded typst-migration-plan.md (#59) (#70)** (auto-sweep). docs(cleanup): delete superseded typst-migration-plan.md (#59) (#70)
  - **fix(archive): static archive cleanup — drop never-shipped Typst plan + dedupe viewer.html (#71)** (auto-sweep). fix(archive): static archive cleanup — drop never-shipped Typst plan + dedupe viewer.html (#71)
  - **Slice 1 — Fix floating dock so it doesn't overlay content (#76)** (auto-sweep). fix(ui): relocate floating dock so it doesn't overlap content (issue #76)
  - **Slice 2 — Mechanical <label for/id> pass on browse + search + soldier form (#77)** (auto-sweep). fix(a11y): associate form labels with inputs via for/id pairs (issue #77)
  - **Slice 3 — Add 420px mobile breakpoint to fix 6px header overflow (#78)** (auto-sweep). fix(ui): cap top-shell width so 6px mobile overflow disappears (issue #78)
  - **Slice 6 — Add pagination/virtualisation to /browse (#81)** (auto-sweep). feat(ui): browse pager with page numbers + First/Last (issue #81)
  - **Slice 7 — Reduce header nav density — hamburger menu below 768px (#82)** (auto-sweep). feat(ui): hamburger nav drawer below 768px (issue #82)
  - **Slice 8 — Fix /compare on mobile (tabindex short-term, stacked cards long-term) (#83)** (auto-sweep). chore: restore audit narrative docs after #83 sweep
  - **Slice 9 — Wrap <details> interactive children (nested-interactive fix) (#84)** (auto-sweep). fix(a11y): resolve nested-interactive violations on soldier detail/edit pages (issue #84)
  - **Slice 10 — Investigate 5.7s /soldiers hydration latency (#85)** (auto-sweep). chore: restore audit narrative docs after #85 sweep
  - **Slice 11 — Audit harness: skip axe on HTMX fragments (#86)** (auto-sweep). refactor(audit): extract shared harness; skip axe on HTMX fragments (issue #86)
  - **Slice 12 — Round 3 audit: interactive flows + populated seed (#87)** (auto-sweep). test(audit): add round 3 verification harness with interactive flow tests (issue #87)
  - **Standardise error response envelope across handlers (#88)** (auto-sweep). feat(appshell): error-envelope round 4 — final sweep (#88)
  - **Add confirmation to bulk destructive actions (#89)** (auto-sweep). feat(ui): confirmation + diff preview on destructive paths (#89)
  - **Make calendar day cells keyboard-accessible (#90)** (auto-sweep). feat(a11y): calendar day cells as buttons with screen-reader labels (#90)
  - **Remove orphan compress-images.exe and tighten repo-root binary hygiene (#91)** (auto-sweep). chore(cleanup): sweep stray root binaries and orphan release zips (#91)
  - **Add GitHub Actions workflows for test, build, and audit harness (#92)** (auto-sweep). ci: add test/build/audit GH Actions workflows + bump-version -VerifyOnly (#92)
  - **Backfill missing docs/migrations/*.md notes for v52-v54 (#93)** (auto-sweep). docs: backfill migration notes v52/v53/v54 (#93)
  - **Fix version-string drift across docs (v1.2.37 vs v1.2.22 vs v1.2.54) (#94)** (auto-sweep). docs: align version strings to current schema (v1.2.54) (#94)
  - **Make stress tests honour -short flag in `make test` (#95)** (auto-sweep). test(stress): honour testing.Short() in stress test files (#95)
  - **Add empty-state CTAs for first-run / truly-empty archives (#98)** (auto-sweep). feat(ui): empty-state CTA partial + first-run Setup card on Calendar (#98)
  - **Fix make render-svg hardcoded user path (#101)** (auto-sweep). fix(make): guard render-svg target when local script absent (#101)
  - **Fill in Makefile help descriptions for undocumented targets (#102)** (auto-sweep). docs(make): add ## descriptions for 7 undocumented targets (#102)
  - **Document recovery/undo stories for every destructive operation (#103)** (auto-sweep). docs: recovery/undo stories for every destructive operation (#103)
  - **Document hidden escape hatches and undocumented features (#104)** (auto-sweep). docs: hidden escape hatches + exhaustive route reference (#104)
  - **CHANGELOG hygiene: backfill v23-v54, group by Added/Changed/Fixed/Removed, add dates (#105)** (auto-sweep). docs: CHANGELOG hygiene — backfill v23-v54, group Unreleased by Keep-a-Changelog (#105)
  - **Add CHECK constraints and magic-string constants for entry_type, evidence_type, archive kind (#106)** (auto-sweep). feat(schema): entry_type discipline + evidence_type rename + constants (#106)
  - **Enforce glossary vocabulary in user-manual (Person Record, Local/Shared/Backup/Static Archive, Claim, Finding) (#108)** (auto-sweep). docs: enforce glossary vocabulary in user-manual + ai-handoff (#108)
  - **Reproducible Typst bootstrap + SHA256-pinned PDFium download (#109)** (auto-sweep). build: reproducible Typst bootstrap + SHA256-pinned PDFium (#109)
  - **Misc cleanup: gitignore consistency, tracked runtime artifacts, stale screenshots, orphan zips, wails build flags (#110)** (auto-sweep). chore(repo): misc cleanup — gitignore, runtime artifacts, AGENTS map, wails -trimpath (#110)
  - **uiids: register panel IDs for Research services + Soldier sub-views (#157)** (auto-sweep). uiids: register panel IDs for Research + Soldier sub-views (closes #157) (#162)
  - **fix(imports): guard handleImportBackup OpenFileDialog (closes #158) (#161)** (auto-sweep). fix(imports): guard handleImportBackup OpenFileDialog (closes #158) (#161)
  - **uiids: register panel IDs for Research + Soldier sub-views (closes #157) (#162)** (auto-sweep). uiids: register panel IDs for Research + Soldier sub-views (closes #157) (#162)
  - **fix(jobs): wire Memorial import log download button (closes #159) (#163)** (auto-sweep). fix(jobs): wire Memorial import log download button (closes #159) (#163)
  - **fix(layout): floating dock no longer overlaps content (closes #160) (#164)** (auto-sweep). fix(layout): floating dock no longer overlaps content (closes #160) (#164)
  - **fix(jobs): remove dead 'Open file' button (closes #166) (#167)** (auto-sweep). fix(jobs): remove dead 'Open file' button (closes #166) (#167)
  - **fix(share-queue): mobile [+ Queue] button needs 44x44 a11y tap target + title label (#203)** (auto-sweep). fix(share-queue): mobile [+ Queue] button needs 44x44 a11y tap target + title label (#203)
  - **fix(browse): mobile card dt labels bumped 0.65rem to 0.75rem for readability (#204)** (auto-sweep). fix(browse): mobile card dt labels bumped 0.65rem to 0.75rem for readability (#204)
  - **fix(docs): correct templ generated-file reality in AGENTS.md and copilot-instructions.md (#205)** (auto-sweep). fix(docs): correct templ generated-file reality in AGENTS.md and copilot-instructions.md (#205)
  - **fix(jobs,appshell): harden shutdown reentrancy + seedArtifactJob race (#206)** (auto-sweep). fix(jobs,appshell): harden shutdown reentrancy + seedArtifactJob race (#206)
  - **fix(appshell): TestOpenDialogGuardRejectsConcurrentDuplicates is timing-flaky under high -count (#207)** (auto-sweep). fix(tests): make dialog-guard tests deterministic (issue #207)
  - **refactor(appshell): migrate handleCalendarPDF debug calls to trace.Log (ADR 0006 Phase 3) (#229)** (auto-sweep). refactor(appshell): migrate handleCalendarPDF debug calls to trace.Log (ADR 0006 Phase 3) (#229)
  - **fix: white-screen on initial load + jobs.jsonl out of data dir (#232)** (auto-sweep). fix: white-screen on initial load + jobs.jsonl out of data dir (#232)
  - **fix(nav): /browse + /share broken UX (lazy-load + + Queue no-op) (#239)** (auto-sweep). fix(nav): lazy-load print-records + install Share Queue at boot (#239)
  - **fix(share-queue): select-all on /share/queue page doesn't toggle per-row checkboxes (#243)** (auto-sweep). fix(share-queue): select-all on /share/queue page doesn't toggle per-row checkboxes (#243)
  - **ux(buttons): add :focus-visible, :active, :disabled states to v2 button primitive (#251)** (auto-sweep). ux(buttons): add :focus-visible, :active, :disabled states to v2 button primitive (issue #251)
  - **ux(theme): absorb new palette from Pallete.png as design tokens (Phase 1: token-only, no visual change) (#252)** (auto-sweep). feat(theme): absorb Pallete.png palette as design tokens (issue #252 Phase 1)
  - **ux(tags): replace survivor_id number input with a select dropdown on /tags merge form (#282)** (auto-sweep). test(audit): smoke probe for /tags merge survivor picker (issue #282 slice 2)
  - **feat(share): extract /share/exports, /share/imports, /share/sync subpages (issue #284, slice 1/3) (#287)** (auto-sweep). feat(share): extract /share/exports, /share/imports, /share/sync subpages (issue #284, slice 1/3) (#287)
  - **Floating-dock Menu panel clips left edge at split-screen viewports (#289)** (auto-sweep). fix(floating-nav): drop split-screen rule that clipped panel left edge (issue #289)
  - **C:/Program Files/Git/anniversary/<month>/<day> direct access renders without app.css (#290)** (auto-sweep). fix(calendar): wrap /anniversary direct access in Layout shell (issue #290)
  - **buildinfo: switch AppVersion to v1.U.N shape (issues #293, #294, #295, #296) (#297)** (auto-sweep). buildinfo: switch AppVersion to v1.U.N shape (issues #293, #294, #295, #296) (#297)
  - **db: enumerate applySchema blocks as Migration slice (issue #273 PR 1 of 2) (#298)** (auto-sweep). db: enumerate applySchema blocks as Migration slice (issue #273 PR 1 of 2) (#298)
  - **cli: ship 'migrate down <target>' (issue #273 PR 2 of 2) (#299)** (auto-sweep). cli: ship 'migrate down <target>' (issue #273 PR 2 of 2) (#299)
  - **update: pre-downgrade snapshot taken automatically before migrate down (issue #273 follow-up) (#300)** (auto-sweep). update: pre-downgrade snapshot taken automatically before migrate down runs (#300)
  - **infra: three-branch model — stable as released-code home, main frozen at 31a8901 (ADR 0009) (#301)** (auto-sweep). infra: three-branch model — stable as released-code home, main frozen at 31a8901 (ADR 0009) (#301)
  - **docs: correct frozen main HEAD reference from 31a8901 to 56e31f0 (#302)** (auto-sweep). docs: correct frozen main HEAD reference from 31a8901 to 56e31f0 (#302)
  - **docs: package-doc synopsis on every Go package (Go Doc audit Phase 1) (#303)** (auto-sweep). docs: add package-doc synopsis to every Go package missing one (Go Doc audit Phase 1) (#303)
  - **docs: fix 6 wrong-starter doc comments (Go Doc audit Phase 2) (#304)** (auto-sweep). docs: fix 6 wrong-starter doc comments (Go Doc audit Phase 2) (#304)
  - **docs: doc comments on high-impact exported types (Go Doc audit Phase 3) (#305)** (auto-sweep). docs: doc comments on high-impact exported types (Go Doc audit Phase 3) (#305)
  - **docs: long-tail sweep + per-package coverage floor (Go Doc audit Phase 4) (#306)** (auto-sweep). docs: long-tail sweep + per-package coverage floor (Go Doc audit Phase 4) (#306)
  - **docs: formalize doc-comment requirement as a CONTEXT.md §Laws entry (#307)** (auto-sweep). docs: formalize doc-comment requirement as a CONTEXT.md §Laws entry (#307)
  - **docs(agents): audit process docs against /tdd /to-issues /grill-me /to-prd + tracer-bullets article (#344)** (auto-sweep). docs(agents): align process docs with /tdd /to-issues + tracer-bullets article (issue #344)
  - **templ: EventTagsListFragment uses fmt.Sprintf URL instead of routebuilder.X(...) for per-chip Detach (#372)** (auto-sweep). routebuilder: add EventTagAttach + EventTagDetach; use in EventTagsListFragment (issue #372)
  - **uiids(soldier): render all 11 soldier-side UIIDs (#397 Slice wide) (#408)** (auto-sweep). uiids(soldier): render all 11 soldier-side UIIDs (#397 Slice wide) (#408)
  - **uiids(event): render PageEvent UIID wrappers (#396) (#409)** (auto-sweep). uiids(event): render PageEvent UIID wrappers (#396) (#409)
  - **audit/orphan-handler-probe: rewrite to scan routebuilder-wired routes (#369) (#410)** (auto-sweep). audit/orphan-handler-probe: rewrite to scan routebuilder-wired routes (#369) (#410)
  - **server-gate /soldiers/{id}* on Event rows (issue #363) (#411)** (auto-sweep). server-gate /soldiers/{id}* on Event rows (issue #363) (#411)
  - **tools/tune: --mode event for Event Record PDF iteration (#358) (#412)** (auto-sweep). tools/tune: --mode event for Event Record PDF iteration (#358) (#412)
  - **Review Queue Resolved tab is a placeholder shell (#461)** (auto-sweep). fix(review-queue): wire Resolved tab to a real listing of resolved findings and merge conflicts (issue #461)
  - **audit: fix smoke.mjs Share export-button probe (pre-#380 #284 drift) (#469)** (auto-sweep). test(audit): fix smoke.mjs Share export-button probe drift (issue #469)
  - **audit: fix 6 pre-existing smoke_articles.mjs failures (refs + revisions + cited-in + tag-merge) (#470)** (auto-sweep). test(audit): fix 6 pre-existing smoke_articles + 2 smoke_tag_merge failures (issue #470)
  - **nav: audit other foldout/mega-menu buttons for the same pink-on-pink / low-contrast text (#473)** (auto-sweep). fix(css): restore opening /* of the Issue #472 / #473 popout comment (debug build break)
  - **feat(typecheck): slice 5c — noImplicitAny JSDoc sweep across frontend/*.js (#478)** (auto-sweep). feat(typecheck): slice 5c — noImplicitAny JSDoc sweep across frontend/*.js (#478)
  - **ci: downgrade race-detector step to advisory — fix in issue #479 (#480)** (auto-sweep). ci: downgrade race-detector step to advisory — fix in issue #479 (#480)
  - **fix(identity): ConfigureUserIdentity has no guard against overwriting an already-complete identity (root cause of THU00 leak into live archive) (#495)** (auto-sweep). fix(identity): ConfigureUserIdentity now refuses to overwrite an already-complete identity (issue #495)
  - **fix(browse): 'Recently added' scope has no recency window — re-imported archive persons surface as recent (#512)** (auto-sweep). fix(browse): 'Recently added' scope now enforces a 7-day recency window (issue #512)
  - **feat(pdf): cap Source Records per Person-Record PDF to one page + warn on truncation (#513)** (auto-sweep). feat(pdf): cap Source Records per Person-Record PDF + warn on truncation (issue #513)
  - **fix(static-archive): printable report duplicates 'Click to view', auto-prints on load, and forces portrait on landscape-designed layout (#519)** (auto-sweep). fix(static-archive): printable report dedupes 'Click to view', drops auto-print, flips landscape (issue #519)
  - **fix(browse): Tags filter silently drops all but the first selected tag — AND logic unreachable (#520)** (auto-sweep). fix(browse): tags filter preserves multi-value form keys (issue #520)
  - **tune: green-baseline + workflow bugs + list-records correctness (issues #515, #516, #517, #518) (#524)** (auto-sweep). tune: green-baseline + workflow bugs + list-records correctness (issues #515, #516, #517, #518) (#524)
  - **feat(markdown): enable goldmark GFM extension so tables + autolinks + task lists render in article preview (#525)** (auto-sweep). feat(markdown): enable goldmark GFM extension for tables + autolinks + strikethrough (issue #525)
  - **ci: green baseline — typecheck fix + mega-menu probe staleness + narrow race step (no more email spam) (#527)** (auto-sweep). ci: green baseline — typecheck fix + mega-menu probe staleness + narrow race step (no more email spam) (#527)
  - **archive + appshell + templates: bug-report bundle image-placeholder mode (issue #545) (#563)** (auto-sweep). archive + appshell + templates: bug-report bundle image-placeholder mode (issue #545) (#563)
  - **audit: Pragmatic Programmer 100-tip review of repo + docs + laws (#567)** (auto-sweep). docs(agents): add 100-tip index appendix to pragmatic-principles.md (issue #567 follow-up)
  - **tests: update 3 pre-existing appshell test needles to match #561 microcopy sweep (#579)** (auto-sweep). tests: update 3 pre-existing appshell test needles to match #561 microcopy sweep (fixes #579)
