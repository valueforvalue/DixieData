...
- **review-queue: Resolved tab now renders a real listing (issue #461)**. `handleReviewQueue` previously ignored `?tab=resolved` and returned the same Open-tab list behind a dashed-border placeholder card (issue #455 slice 4 commit shipped the tab strip + the placeholder but never the data path). Two new service methods land: `AuditService.ListResolvedFindings(page, pageSize)` (joins `duplicate_audit_findings` LEFT JOIN `soldiers` for the cached display IDs, newest-first by `resolved_at`) and `BackupService.ListResolvedConflicts(page, pageSize)` (over `merge_review_conflicts WHERE resolution IS NOT NULL`, newest-first). The handler now dispatches on `tab`: Open branch unchanged; Resolved branch reads both streams + renders the new `ResolvedReviewQueueView` templ (`internal/templates/review_queue.templ`) which lists findings + conflicts as separate rows with the green "Resolved" pill, links back to each Person Record, and routes per-row to `/review-queue/compare/{id}` (findings) or `/soldiers/{id}#ledger` (conflicts). Model gained `MergeReviewConflict.ResolvedAt` so the resolved-at timestamp propagates end-to-end. RED-first regression net: `TestAuditServiceListResolvedFindingsPaginated` in `internal/records/audit_service_test.go` seeds an audit + resolves one finding + asserts the resolved row surfaces with cached display IDs + a populated `resolved_at` + the open pair stays out; `TestReviewQueueResolvedTabDispatchesAwayFromOpen` + `TestReviewQueueResolvedTabEmptyStillRenders` in `internal/appshell/review_queue_resolved_test.go` hit the live HTTP server + assert the two tab response bodies differ + the empty-state copy renders when no resolved entries exist. Wider sweep: `go test -short -count=1 ./internal/...` all green modulo the pre-existing `internal/archive` PDF/JPG/typst-version failures.
