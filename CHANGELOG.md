...
- **ui: startup placeholder reads persisted theme from disk on cold start (issue #481 follow-up, issue #483)**. The #481 fix only worked once `App.Startup()` had stored the resolved theme in the `a.theme` atomic. But the Wails WebView2 fires its first HTTP request BEFORE `Startup()` runs — at that moment `a.theme` is a zero-value `atomic.Value` (`Load()` returns nil) and the placeholder's nil-safe fallback hardcoded `data-theme="default"` even when the user had persisted `theme="high-contrast"` on disk. Symptom: High Contrast / Soft users saw a flash of the Default-theme loading card on every cold launch until the 700ms JS redirect hit the warmed mux and `lifecycle.go:483` read the now-stored atomic (the "theme changes after navigating away" report). The placeholder now backfills from `local_settings.json` via `records.LoadLocalSettings(a.dataDir).ResolvedTheme()` when the atomic is nil — the disk read happens only during the cold-start race window; once `Startup()` stores the atomic, every later request (including the placeholder's own refresh) hits the in-memory path. Empty `dataDir` (a truly fresh `NewApp()` with no settings file) and load errors fall through to `records.ThemeDefault` so the existing `uninitialized app falls back to default theme` sub-case still holds. RED-first regression net: new `TestStartupPlaceholderReadsPersistedThemeFromDiskOnColdStart` (3 sub-cases — persisted High Contrast, persisted Soft, missing settings file falls back to default) drives the actual race path (`NewApp()` → `ServeHTTP` with `mux==nil` → `renderStartupPlaceholder`) and asserts the user's exact symptom. `go test -short -count=1 ./internal/appshell/...` all green.
