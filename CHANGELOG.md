...
- Image compression service (issue #73): re-encodes stored images to JPEG q=85 on import; `cmd/compress-images` binary for batch backfill; Settings card to scan and run the batch; original files retained in `dataDir/temp_trash/compressed/` for 30 days.
