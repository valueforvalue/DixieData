...
- **Source Record links: collapse long URLs into 'Click to view' anchor on live surfaces (issue #541)**. Source Record `details` fields rendered the raw URL (60–120 chars typical) as visible anchor text on the live Person Record detail page (`/soldiers/{id}` in the Wails app) and on the static archive's live `#/soldiers/{displayId}` view. Both surfaces now match the PDF export and the printable HTML export, which already collapse the URL into a "Click to view" anchor. The new `internal/templates/record_details_support.go` Go helper + a sibling JS `renderRecordDetailsLink` helper share a single contract: single http(s) URLs become `<a target="_blank" rel="noreferrer noopener">Click to view</a>` with the URL hidden from visible text; trailing punctuation (one character, set `.,;:!?)]}`) detaches from the anchor; non-URL text passes through unchanged; >4000-char URLs and non-http(s) schemes (`javascript:`, `ftp:`, `file:`) fall through to plain text. The Wails surface uses the new `RecordDetails` templ component (slice 2); the static archive uses the new JS helper at the live detail rendering site (slice 3). `LinkedText` is untouched — biography, notes, and freeform text still show visible URLs per the scope decision in issue #541. RED-first regression nets: `internal/templates/record_details_support_test.go` pins the linkifier contract (single URL, trailing punct detach, freeform passthrough, http(s)-only scheme gate, 4000-char upper bound, canonical anchor text); `TestSoldierDetailRendersClickToViewAnchorOnURLDetails` in `internal/templates/soldier_card_test.go` pins the Wails-side rendering end-to-end (anchor copy, hidden URL, target/rel attrs, trailing-period detach, freeform-text row); `audit/smoke_static_archive_revamp.mjs` gains four source-scan assertions on `renderRecordDetailsLink` mirroring the printable-export's `printRenderLink` assertion shape. `go test -short -count=1 ./...` green; the static-archive probe exits 0.
