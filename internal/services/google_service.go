...
func googleCalendarEvent(soldier models.Soldier) *gcal.Event {
	startYear := soldier.DeathYear
	if startYear < 1 {
		startYear = time.Now().Year()
	}
	start := time.Date(startYear, time.Month(soldier.DeathMonth), soldier.DeathDay, 0, 0, 0, 0, time.UTC)
	end := start.AddDate(0, 0, 1)

	description := fmt.Sprintf("Database Number: %s\nUnit: %s\nBuried In: %s\nGenerated by DixieData.", soldier.DisplayID, soldier.Unit, soldier.BuriedIn)
	return &gcal.Event{
		Summary:     fmt.Sprintf("DixieData Anniversary: %s", googleSoldierDisplayName(soldier)),
		Description: description,
		Start:       &gcal.EventDateTime{Date: start.Format("2006-01-02")},
		End:         &gcal.EventDateTime{Date: end.Format("2006-01-02")},
