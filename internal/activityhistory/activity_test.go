...
// TestTopContributorsCapsAt10 pins the slice cap. Contributors
// beyond the 10th are dropped.
func TestTopContributorsCapsAt10(t *testing.T) {
	...
	// Sorted descending; the highest commit count is "c" + 19.
	if got[0].Name != "c" || got[0].Count != 19 {
		// Names are 1-char; can verify count instead.
		if got[0].Count != 19 {
...
// one bucket per canonical Type label. Empty archive -> empty
// map (not nil) so the templ partial can iterate.
func TestIssuesClosedBucketShape(t *testing.T) {
	got := IssuesClosedFromLabels([]IssueLabel{
		...
		{Name: "documentation"},
		{Name: "wontfix"},
		{Name: "area:frontend"}, // not a Type label -> dropped
		{Name: "needs-triage"},  // not a Type label -> dropped
	})
