...
func (g *GoogleService) SyncTestCalendar(ctx context.Context) (GoogleCalendarSyncResult, error) {
	...
	for _, synthetic := range syntheticTestEvents() {
		event := synthetic.toCalendarEvent(calendarTimeZone)
...
func googleCalendarTimeZone(ctx context.Context, calSvc *gcal.Service, calendarID string) (string, error) {
	...
	calendar, err := calSvc.Calendars.Get(calendarID).Fields("timeZone").Context(ctx).Do()
	if err != nil {
		return "", err
	}
	if calendar != nil {
...
func googleCalendarLocation(timeZone string) *time.Location {
	location, err := time.LoadLocation(normalizeGoogleCalendarTimeZone(timeZone))
	if err != nil {
		return time.UTC
	}
	return location
...
func (e googleSyntheticEvent) toCalendarEvent(timeZone string) *gcal.Event {
	timeZone = normalizeGoogleCalendarTimeZone(timeZone)
	location := googleCalendarLocation(timeZone)
	start := time.Now().In(location).AddDate(0, 0, e.OffsetDay)
	start = time.Date(start.Year(), start.Month(), start.Day(), 10, 0, 0, 0, location)
