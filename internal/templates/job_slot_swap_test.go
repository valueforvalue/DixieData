...
// in the layout; uiids.OverlayJobsProgress). The slot fragment
// (job_slot_fragment.templ) targets it with
// `hx-target="[data-jobs-progress-region]"` so subsequent polls
...
// div that lacks the `data-jobs-progress-region` attribute. Every
// poll thereafter hits `htmx:targetError, [data-jobs-progress-region]`
// and the visible progress freezes at the value captured in the
// first poll. This test guards the swap strategy by parsing the
// rendered fragment HTML and asserting innerHTML is in use.
package templates
...
// layout's persistent progress region (data-jobs-progress-region).
// If a future commit regresses this to outerHTML, the "stuck at
// 5%" symptom returns: the layout's data-jobs-progress-region
// div gets replaced with the fragment's <div class="card"> on the
// first poll, and every subsequent poll fails to find the target.
func TestJobStatusSlotFragmentUsesInnerHTMLSwap(t *testing.T) {
	...
	if !strings.Contains(html, `hx-target="[data-jobs-progress-region]"`) {
		t.Errorf("fragment must target the persistent progress region [data-jobs-progress-region]; got:\n%s", html)
	}
	if !strings.Contains(html, `hx-swap="innerHTML"`) {
		t.Errorf("fragment must use hx-swap=innerHTML against the layout's persistent progress region; outerHTML strips the target's data-jobs-progress-region attribute on the first poll, causing htmx:targetError and stuck-at-5-percent regressions. Rendered HTML:\n%s", html)
	}
	if strings.Contains(html, `hx-swap="outerHTML"`) {
		t.Errorf("fragment must NOT use hx-swap=\"outerHTML\" against [data-jobs-progress-region]; see TestJobStatusSlotFragmentUsesInnerHTMLSwap doc comment. Rendered HTML:\n%s", html)
	}
}
