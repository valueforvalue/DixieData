// parse_test.go -- issue #585 slice 2.
//
...
// the parser behaviour end-to-end.
//
// Fixture-driven: the parser is tested against a literal
// CHANGELOG fragment that exercises both the pre-#266
// `## v1.2.N` shape AND the post-#266 `## v1.U.N` shape
// plus the empty-state ([Unreleased] only) and a single-line
...
// fixtureCHANGELOG exercises every shape the parser must
// handle.
//   - Two `### Added` sections, one `### Changed`, one
//     `### Fixed`.
//   - Two distinct headings: the legacy `## v1.2.55 - 2026-06-25`
//     shape AND the post-#266 `## v1.1.4 - 2026-07-12`
//     shape.
...
// TestParseReaderExcludesUnreleased pins that the
// [Unreleased] block is never surfaced as a release entry.
// The templ partial reads baked entries only; [Unreleased] is
// the author's working buffer.
...
// releases, no error. The dev binary ships baked == nil and
// the templ partial must render an empty-state copy, not
// crash.
...
// A parser that requires N+ bullets per subsection fails
// here.
