...
func runOutputAudit(reportDir string) (report, error) {
	...
	defer debug.DeferCloseLog(restoredDB, "runOutputAudit.restoredDB")
...
func readZipWithManifest(path string) (map[string][]byte, archive.BackupManifest, error) {
	...
	defer debug.DeferCloseLog(reader, "readZipWithManifest.reader")
...
func sqliteUserVersion(path string) (int, error) {
	...
	defer debug.DeferCloseLog(conn, "sqliteUserVersion.conn")
	var version int
