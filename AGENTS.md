...
- **Before pushing:** `make test` (runs `go test ./... -short`) and `make tpl` (regenerates `internal/templates/*_templ.go` from the `.templ` sources — these generated files are **gitignored**, so the diff after `make tpl` only shows up in your working tree, never in the PR. CI regenerates them in the workflow step before tests run). If you touched htmx or templ markup, run `node audit/smoke.mjs` against a live `dixiedata-web` server. `make audit` runs the full visual sweep.
