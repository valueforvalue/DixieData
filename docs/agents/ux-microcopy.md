...
### iCalendar coverage (issue #581)

`audit/smoke_icalendar_microcopy.mjs` walks the only Go file
that emits iCalendar copy — `internal/archive/export_service.go`
— and pins the canonical user-facing strings verbatim. RFC 5545
is a different grammar from Typst, Go-templ, and HTML, so the
existing three probes do not apply; the iCalendar probe is
format-aware. It checks:

- `X-WR-CALNAME:DixieData Memorial Anniversaries` (glossary-
  aligned product noun; preserved verbatim per user direction);
- `PRODID:-//…//Memorial Anniversaries v{…}//EN` (RFC 5545
  §3.7.3 producer identifier);
- SUMMARY presets: `"{name} Memorial Anniversary"`,
  `"{DisplayID} • {name}"`,
  `"Memorial Anniversary: {name}"`;
- DESCRIPTION lines: `"Record ID: "`, `"Unit: "`,
  `"Buried In: "`, `"Original Death Date: "`, and the
  trailing `"Generated by DixieData."` attribution (kept
  per user direction);
- VALARM bodies: `"Upcoming memorial anniversary for {name}"`
  and `"Memorial anniversary in one hour for {name}"`.

Drift signposts (must NOT appear): non-canonical calname,
`"Birthday"` / `"Death Anniversary"` / `"Memorial Day for "` in
SUMMARY, accidental `.ics` suffix on calname.

The probe skips machine-readable envelope extensions
(`X-DIXIEDATA-*`), UID generation, the `icalText()` escape
helper, the `writeICalendarLine` byte writer, and the CSV /
JSON / JPG export paths. **Slice 3 ships GREEN-on-HEAD (no
edits planned)**; the catalog is the freeze state. Any future
contributor who wants to drift a pinned string must update the
catalog + the slice plan + cite #581 in their commit message.

When changing iCalendar copy, run:
