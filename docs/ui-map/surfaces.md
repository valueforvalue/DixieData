...
| `panel.article.markdown-cheatsheet` | panel | Article editor Markdown syntax cheatsheet popover (issue #565 + #610); Foldout-based panel (issue #264 primitive) triggered by `data-article-md-cheatsheet-open`. Anchored top-right of the editor toolbar. Each row carries an Insert-at-cursor menuitem + a Copy example button + a live preview cell. |
