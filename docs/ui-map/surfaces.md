...
| `panel.article.markdown-cheatsheet` | panel | Article editor Markdown syntax cheatsheet popover (issue #565 + #610); Foldout-based panel (issue #264 primitive) triggered by `data-foldout-trigger="panel.article.markdown-cheatsheet"` (the canonical Foldout trigger marker — see `internal/templates/components/foldout.templ` for the primitive's trigger contract). Issue #704 corrected an earlier wireframe typo that mis-documented the trigger marker. Anchored top-right of the editor toolbar. Each row carries an Insert-at-cursor menuitem + a Copy example button + a live preview cell. |
