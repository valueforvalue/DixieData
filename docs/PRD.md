...
- **Phase 2 — remaining exports and minimal static archive unification.** `anniversary.typ`, `analytics_summary.typ`, `printable_archive_registry.typ`. The static archive is migrated to Typst HTML output as a thin slice — the goal is to unhook the static archive from the duplicated `internal/archive/static_archive.go` design system, not to preserve every visual and interactive nuance. The static archive has its own audit and overhaul coming (see Further Notes), so this slice accepts quirks. The existing interactive JS (search filter, image overlay zoom) is **deferred** to the static-archive overhaul. After this slice, the static archive HTML is generated by Typst but may look different, have different structure, and have reduced interactivity.
