...
- **Slice 6 (Phase 2b) — static archive unification. Deferred indefinitely.** Typst HTML output would have replaced the existing `staticArchiveIndexHTML` constant in `internal/archive/static_archive.go` and removed the 27 alpha-variant rgba duplications, but the static archive's interactive JS (search, image overlay zoom) is the feature's primary value-add — Typst HTML output is static. The slice was therefore **not intended** to ship. `templates/static_archive_index.typ` was scaffolded speculatively and has since been deleted (June 2026); `internal/archive/static_archive.go::staticArchiveIndexHTML` remains the source of truth. The static-archive overhaul (CSS-variable unification, alpha-variant rgba reduction, design-system consolidation with the live app) is tracked as separate follow-up work.
