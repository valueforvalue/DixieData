...
(function() {
    ...
    function exportLedger() {
        ...
        // Issue #383 Slice 1: wrap the ledger in the v1 envelope
        // so the DixieData importer can detect format drift,
        // refuse on major bumps, and warn on minor bumps. The
        // DixieData side parses envelope + falls back to a bare
        // array for pre-v1 archives, so users on an older
        // scraper still get their data imported (with a warning).
        //
        // format_version = "memorial_v1" — bump the MAJOR when
        // the entry shape changes (fields renamed, types
        // changed, structure restructured); bump the MINOR when
        // the shape is additive (new field, optional field).
        // script_version mirrors the @version metadata so the
        // DixieData summary surfaces which scraper release
        // produced the file.
        //
        // NOTE: DixieData stamps its own app_version on import
        // (not here — this script doesn't know what DixieData
        // version is installed). The DixieData import summary
        // carries both the scraper's script_version + the
        // DixieData importer's app_version so future audits can
        // correlate "which scraper + which DixieData build
        // produced this row".
        const envelope = {
            format_version: "memorial_v1",
            script_version: "1.0",
            script_name: GM_info && GM_info.script && GM_info.script.name ? GM_info.script.name : "Find A Grave Ambient Scraper",
            entries: ledger
        };
