...
8. **Static-before-wildcard route order for `/share/*`** — chi matches in registration order. `/share` exists as `r.Get("/share", a.handleShare)` in `routes.go`. Any new literal route under `/share` (e.g. `/share/queue`, `/share/queue/presets`) MUST register before the `/share` wildcard handler. Issue #193 had to move `/share/queue` above the wildcard or chi would resolve `/share/queue` to `handleShare` and serve the wrong page. (Issue #310 deleted the modal that lived at `/share/queue/modal`; the route-order lesson still applies to any future literal routes under `/share*`.)
