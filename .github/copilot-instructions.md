...
- The frontend is server-rendered. `internal\templates\*.templ` defines the HTML; generated `*_templ.go` files are **gitignored** and regenerated locally by `just tpl` and in CI by the test/audit workflows. `frontend\app.js` is a custom HTMX-style request/swap layer that drives navigation, form submissions, Smart Back, toasts, tabs, and other rich interactions.
...
- Template changes are two-part changes: edit the `.templ` file and run `just tpl` to regenerate the gitignored `*_templ.go` output. CI regenerates from the committed `.templ` source before `go test` and `audit` runs.
